...
static void connectUntoggleAction(GtkWidget* btn) {
    GAction* action = findAction(GTK_ACTIONABLE(btn));
    if (!action) {
        return;
    }
    g_signal_connect_object(btn, "toggled", G_CALLBACK(+[](GtkToggleButton* btn, gpointer a) {
...
void setToggleButtonUnreleasable(GtkToggleButton* btn) {
    if (GTK_IS_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(btn)))) {
        connectUntoggleAction(GTK_WIDGET(btn));
    } else {
