...
MuPdfExport::MuPdfExport(const Document* doc, ProgressListener* progressListener):
...
bool MuPdfExport::overlayAndSave(const fs::path& saveDestination, std::stringstream& overlaystream,
    ...
    try {
        ...
        background.super().fz_set_metadata(FZ_META_INFO_PRODUCER,
                                           (std::string(PROJECT_STRING) + " + muPDF " + FZ_VERSION).data());
        background.super().fz_set_metadata(FZ_META_INFO_MODIFICATIONDATE, createPDFDateStringForNow().data());
