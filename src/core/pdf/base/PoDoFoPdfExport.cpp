...
PoDoFoPdfExport::PoDoFoPdfExport(const Document* doc, ProgressListener* progressListener):
        HybridPdfExport(doc, progressListener) {}

PoDoFoPdfExport::~PoDoFoPdfExport() = default;

/// Ensure this given object is either Indirect or a Reference (within the provided Document)
static PoDoFo::PdfObject& ensureIndirect(PoDoFo::PdfObject& o, PoDoFo::PdfDocument& doc) {
    if (o.IsIndirect() || o.IsReference()) {
        return o;
    }
    o = doc.GetObjects().CreateObject(std::move(o)).GetIndirectReference();
    return o;  // o is now a PdfReference
}
...
static PoDoFo::PdfPage& duplicatePage(PoDoFo::PdfPage& source, unsigned int insertPosition) {
    ...
    auto& clone = document.GetPages().CreatePageAt(insertPosition, source.GetMediaBox());

    // Shallow clone everything in the dictionary
    for (auto& e: source.GetDictionary()) {
        if (e.first == PoDoFo::PdfName("Parent") || e.first == PoDoFo::PdfName("Type") ||
            e.first == PoDoFo::PdfName("MediaBox")) {
            continue;  // Those are handled by PoDoFo::PdfPageCollection::CreatePageAt()
        }
        if (e.first == PoDoFo::PdfName("Resources") || e.first == PoDoFo::PdfName("Contents")) {
            continue;  // Those are handled differently to allow easy data appending - see below
        }
        clone.GetDictionary().AddKeyIndirectSafe(e.first, ensureIndirect(e.second, document));
    }
        ...
        if (cloneContents.IsArray()) {
            ...
            if (ob.IsArray()) {
                for (auto& y: ob.GetArray()) {
                    ...
                    array.AddIndirectSafe(ensureIndirect(y, document));
                    // Nb: When the page is overlayed (see overlayPage() below), PoDoFo::PdfPainter will flatten the
                    // contents and hence copy the content to the duplicated page. The above ensureIndirect() is only
                    // useful if there are unannotated and duplicated background.
                }
    ...
    // Resources
    auto& resourcesP = source.GetResources();
    auto& resourcesQDict = clone.GetResources().GetDictionary();
    for (auto&& a: resourcesP.GetDictionary()) {
        resourcesQDict.AddKeyIndirectSafe(a.first, ensureIndirect(a.second, document));
    }
    return clone;
}
...
bool PoDoFoPdfExport::overlayAndSave(const fs::path& saveDestination, std::stringstream& overlaystream,
    ...
    try {
        ...
        output.GetMetadata().SetProducer(
                PoDoFo::PdfString(std::string(PROJECT_STRING) + " + PoDoFo " + PODOFO_VERSION_STRING));
        // ModDate is updated automatically by PoDoFo
