...
EllipseHandler::EllipseHandler(XournalView* xournal, const PageRef& page, bool flipShift, bool flipControl):
        BaseShapeHandler(xournal, page, flipShift, flipControl) {}

EllipseHandler::~EllipseHandler() = default;

auto EllipseHandler::createShape(const PositionInputData& pos) -> std::pair<std::vector<Point>, Range> {
    /**
