...
auto SplineSegment::getCoarseBoundingBox() const -> Rectangle<double> {
    ...
    // Avoid returning a rectangle with null width/height
    // This avoids bugs with eraser on purely vertical/horizontal spline segments
    return Rectangle<double>(intervalX.min, intervalY.min, std::max(intervalX.length(), MIN_BOX_SIZE),
...
auto SplineSegment::getThickBoundingBox() const -> Rectangle<double> {
    /**
