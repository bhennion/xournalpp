...
static auto newTextColorIcon(Color color) {
    ...
    stream << "<svg width=\"24\" height=\"24\" stroke-width=\"1\" stroke-linecap=\"round\" stroke-linejoin=\"round\" "
              "xmlns=\"http://www.w3.org/2000/svg\">"
              "<text x=\"12\" y=\"15\" fill=\"black\" font-weight=\"bold\" font-size=\"20\" "
              "text-anchor=\"middle\">A</text>"
              "<rect width=\"20\" height=\"4\" x=\"2\" y=\"18\" rx=\"1\" ry=\"1\" "
              "style=\"stroke:#000000;stroke-opacity:1;fill:#"
           << std::hex << std::setw(6) << std::setfill('0') << (uint32_t(color) & 0x00ffffff) << "\"/></svg>";
    ...
    auto icon = xoj::util::GObjectSPtr<GdkPixbuf>(
            gdk_pixbuf_new_from_stream(gstream.get(), NULL, xoj::util::out_ptr(err)), xoj::util::adopt);
...
TextEditorContextMenu::TextEditorContextMenu(Control* control, TextEditor* editor, XojPageView* pageView,
        ...
        color(editor->getTextElement()->getColor()) {
    // Only for debugging
...
void TextEditorContextMenu::changeColor() {
    XojMsgBox::showColorChooserDialog(
            this->control->getGtkWindow(), _("Choose text color"), this->color, [this](std::optional<Color> c) {
                if (c.has_value()) {
                    this->color = c.value();
                    auto colorU16 = Util::argb_to_ColorU16(this->color);
                    this->editor->addInlineAttribute(xoj::util::PangoAttributeUPtr(
                            pango_attr_foreground_new(colorU16.red, colorU16.green, colorU16.blue)));
...
void TextEditorContextMenu::toggleItalic() {
    ...
    this->editor->addInlineAttribute(xoj::util::PangoAttributeUPtr(
            pango_attr_style_new(this->italic ? PANGO_STYLE_ITALIC : PANGO_STYLE_NORMAL)));
    gtk_widget_grab_focus(this->xournalWidget);
}

void TextEditorContextMenu::toggleBold() {
    this->bold = !this->bold;
    this->editor->addInlineAttribute(
            xoj::util::PangoAttributeUPtr(pango_attr_weight_new(this->bold ? PANGO_WEIGHT_BOLD : PANGO_WEIGHT_NORMAL)));
    gtk_widget_grab_focus(this->xournalWidget);
}
...
void TextEditorContextMenu::toggleUnderline() {
    ...
    this->editor->addInlineAttribute(xoj::util::PangoAttributeUPtr(
            pango_attr_underline_new(this->underlined ? PANGO_UNDERLINE_SINGLE : PANGO_UNDERLINE_NONE)));
    gtk_widget_grab_focus(this->xournalWidget);
}
...
template <typename PangoEnum>
static PangoEnum getEnumAttr(PangoAttribute* p) {
    return static_cast<PangoEnum>(pango_attribute_as_int(p)->value);
}
...
void TextEditorContextMenu::setAttributes(std::vector<xoj::util::PangoAttributeUPtr> attributes) {
    ...
    for (auto&& p: attributes) {
        switch (p->klass->type) {
            case PANGO_ATTR_FONT_DESC:
                gtk_font_chooser_set_font_desc(GTK_FONT_CHOOSER(this->fontBtn),
                                               pango_attribute_as_font_desc(p.get())->desc);
                inlineFont = true;
                break;
    ...
    if (!inlineFont) {
        ...
        xoj::util::PangoFontDescriptionUPtr desc(
                pango_font_description_from_string(editor->getTextElement()->getFontName().c_str()));
