...
ToolbarBox* get_toolbar_box(GtkWidget* w) {
    return IS_TOOLBAR_BOX(w) ? TOOLBAR_BOX(w)->parent : nullptr;
}
...
void ToolbarBox::startEditing(ToolMenuHandler* handler) {
    ...
    // prepare drag & drop
    auto* target = gtk_drop_target_new(G_TYPE_STRING, GdkDragAction(GDK_ACTION_COPY | GDK_ACTION_MOVE));
    ...
    g_signal_connect(target, "enter",
                     G_CALLBACK((+[](GtkDropTarget* target, gdouble x, gdouble y, gpointer) -> GdkDragAction {
                         ToolbarBox* toolbar =
                         ...
                         auto [item, proxy] = data.handler->createItem(g_value_get_string(&val), side);
                         data.tempItem = item;

                         g_message("Drop::enter: Created %s at %p", g_value_get_string(&val), item.get());
                         ...
                         toolbar->children.emplace(std::next(toolbar->children.begin(), data.tempItemPosition),
                                                   std::move(item), std::move(proxy));
    ...
    g_signal_connect(
            ...
            G_CALLBACK(+[](GtkDragSource* source, double x, double y, gpointer) -> GdkContentProvider* {
                ...
                return gdk_content_provider_new_typed(G_TYPE_STRING, id.c_str());
            }),
            nullptr);
...
auto ToolbarBox::endEditing() -> ToolbarEntry {
    ...
    auto toolbarName = editingData->handler->getControl()->getWindow()->getToolbarName(getWidget());
    editingData.reset();
