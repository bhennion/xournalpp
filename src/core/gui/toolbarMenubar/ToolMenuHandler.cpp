...
static void setWidgetId(GtkWidget* item, const std::string_view& name) {
    g_object_set_data_full(G_OBJECT(item), TOOLITEM_ID_PROPERTY, new std::string(name), xoj::util::destroy_cb<std::string>);
}

auto ToolMenuHandler::createItem(const char* id, ToolbarSide side) const -> std::pair<xoj::util::WidgetSPtr, xoj::util::WidgetSPtr> {
    ...
    if (StringUtils::startsWith(name, "COLOR(") && StringUtils::endsWith(name, ")")) {
        ...
        const NamedColor& namedColor = this->control->getSettings()->getColorPalette().getColorAt(paletteIndex);
